...
feats.bm.intersect.b4 <- vector(mode="list",length=length(SF.List))
feats.intersect.b4 <- vector(mode="list",length=length(SF.List))
...
for(i in 1:length(SF.List)){
  ...
  feats.bm.intersect.b4[[i]] <- c(featSummary$Feature[which(featSummary$Frequency >= K*SF.List[i])])
  ...
  feats.intersect.b4[[i]] <- c(feats.int[feats.int != "y"])
}

Stop.Time <- Sys.time()
Total.P1.B4.Time <- difftime(Stop.Time, Start.Time)

save(feats.bm.intersect.b1,file="P1B4_LASSOI_Feats.RData")
save(feats.intersect.b1,file="P1B4_SOFSI_Feats.RData")
